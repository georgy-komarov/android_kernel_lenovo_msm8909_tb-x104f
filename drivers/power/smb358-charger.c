...
struct proc_dir_entry *battery_type_entry;
char	*battery_name;
static int battery_proc_show(struct seq_file *seq, void *v)
{
	seq_puts(seq, (const char *)battery_name);
	return 0;
}

static int battery_proc_open(struct inode *inode, struct file *file)
{
 return single_open(file, battery_proc_show, inode->i_private);
}

static const struct file_operations battery_type_fops = {
 .open  = battery_proc_open,
 .read  = seq_read,
// .write  = seq_write,
 .llseek  = seq_lseek,
...
static int smb358_otg_set_limit_current(struct smb358_charger *chip)
{
	int rc;
	u8  reg;
	rc = smb358_masked_write(chip, OTG_CONTROL, SMB358_MASK(3,2),
						0xC);
	if (rc)
		dev_err(chip->dev, "Couldn't write OTG_CONTROL rc=%d\n",
				rc);
	rc = smb358_read_reg(chip, OTG_CONTROL, &reg);
	pr_err("smb358 otg set control %2x\n",reg);
	return rc;
}
...
#define OTG_PROC_FILE "smb358_otg_enable"

int otg_enable = 0;
extern int hq_otg_read_state(void);

struct smb358_charger *this_chip = NULL;

static struct proc_dir_entry *smb358_otg_proc = NULL;

static int smb358_otg_read_proc(struct seq_file *m, void *data)
{
	seq_printf(m, "%d\n", otg_enable);
	return 0;
}

static int smb358_otg_write_proc(struct file *filp, const char __user *buffer, size_t count, loff_t *off)
{
	int rc = 0, retry = 0;
	unsigned int cmode;
	...
	if (kstrtol(tmp, 10, (long *)&cmode) == 0) {
		if(cmode == 1 || cmode == 0)
		{
			otg_enable = cmode;
		}
	...
	if(otg_enable)
	{
		if(hq_otg_read_state() < 2)
		{
			for(retry = 3; retry > 0; retry--)
			{
				rc = smb358_masked_write(this_chip, CMD_A_REG, CMD_A_OTG_ENABLE_BIT, CMD_A_OTG_ENABLE_BIT);
				if (rc)
				{
					printk("[guoliang]Couldn't enable OTG mode rc=%d, reg=%2x\n", rc, CMD_A_REG);
				} else {
					break;
				}
			}
		...
		for(retry = 3; retry > 0; retry--)
		{
			rc = smb358_masked_write(this_chip, CMD_A_REG, CMD_A_OTG_ENABLE_BIT, 0);
			if (rc)
			{
				printk("[guoliang]Couldn't disable OTG mode rc=%d, reg=%2x\n", rc, CMD_A_REG);
			} else {
				break;
			}
		}
...
static int smb358_otg_open_proc(struct inode *inode, struct file *file)
{
	struct smb358_charger *chip = inode->i_private;

	return single_open(file, smb358_otg_read_proc, chip);
}

static const struct file_operations smb358_otg_proc_ops = {
	.owner = THIS_MODULE,
	.open = smb358_otg_open_proc,
	.release = single_release,
    .llseek = seq_lseek,
	.read  = seq_read,
	.write = smb358_otg_write_proc,
};
...
static int smb358_regulator_init(struct smb358_charger *chip)
{
	...
	this_chip = chip;
  	...
  	if (smb358_masked_write(this_chip, CMD_A_REG, CMD_A_OTG_ENABLE_BIT, 0))
	{
          	printk("[guoliang]Couldn't disable OTG mode reg=%2x\n", CMD_A_REG);
	}
...
#define OTG_FREQUENCY	0X80
#define SWITCHING_FREQUENCY BIT(7)
static int smb358_hw_init(struct smb358_charger *chip)
{
	...
	rc = smb358_otg_set_limit_current(chip);
	if (rc < 0)
		pr_err("set otg current limit fail rc =%d\n",rc);
	...
	rc = smb358_masked_write(chip, THERM_A_CTRL_REG,
		THERM_A_THERM_MONITOR_EN_MASK | SWITCHING_FREQUENCY, OTG_FREQUENCY);
	if (rc) {
...
static int smb358_get_prop_batt_current_now(struct smb358_charger *chip)
{
	...
	dev_dbg(chip->dev,
		"Couldn't get bms_psy, Failed, Retunr -22\n");
	return -22;
}
...
//Modify by duanhanxing for thermal charger (8909)  20180316  begin
		...
		int lvl_sel)
{
	...
	if (chip->therm_lvl_sel > 0) {
		rc = smb358_fastchg_current_set(chip, 450);
		if (rc)
		dev_err(chip->dev,
...
//Modify by duanhanxing for thermal charger (8909)  20180316  end
				       ...
				       union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_BATTERY_ID:
		...
		break;
	//Modify by duanhanxing for thermal charger (8909)  20180315  begin	
	case POWER_SUPPLY_PROP_SYSTEM_TEMP_LEVEL:
...
static int chg_uv(struct smb358_charger *chip, u8 status)
{
	...
	if (rc < 0) {
		dev_err(chip->dev,
			"Couldn't set charging enable in chg_uv rc = %d\n",rc);
	}
	power_supply_changed(chip->usb_psy);
...
static int smb_parse_dt(struct smb358_charger *chip)
{
	...
	//Modify by duanhanxing for thermal charger (8909)  20180315  end
...
/* Modify by duanhanxing for battery debug (8909) 20180313 start */
char *battery_manufacturer_info = "Unknow";
static int64_t read_battery_id(struct smb358_charger *chip)
{
	int rc;
...
/* Modify by duanhanxing for battery debug (8909) 20180313 end */
#if defined(CONFIG_DEBUG_FS)
				...
				const struct i2c_device_id *id)
{
	...
	struct power_supply *usb_psy;
	u8 reg = 0;
	smb358_otg_proc = proc_create(OTG_PROC_FILE, 0666, NULL, &smb358_otg_proc_ops);
	...
	battery_type_entry = proc_create("app_info", 0666, NULL, &battery_type_fops);
	if(battery_type_entry == NULL)
	{
		pr_err("proc create battery type fail\n");
		return -ENOMEM;
	}
	...
	if((read_battery_id(chip) > 500000))
	{
		...
		battery_name = "battery_name:FMT\n";
		battery_manufacturer_info = "battery manufacturer:FMT";

	}else{
		chip->battery_id_info = 2;
		chip->battery_missing = 0;
        battery_name = "battery_name:SWD\n";
        battery_manufacturer_info = "battery manufacturer:SWD";
    }
	/* Modify by duanhanxing for battery debug (8909)  20180313 end */
