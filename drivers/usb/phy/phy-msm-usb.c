...
static int dcp_max_current = DCP_CHG_MAX;
...
#define ID_T_HUB_WITHOUT_POWER_HIGH 600*1000//0~0.4, normally it's around 0.021V

#define ID_T_HUB_WITH_POWER_HIGH 1200*1000//0.4~0.9, normally 0.544~0.68
...
static int msm_hsusb_ldo_init(struct msm_otg *motg, int init)
{
	...
	if (init) {
		...
		rc = regulator_set_voltage(hsusb_gpiopull, USB_PHY_1P8_VOL_MIN,USB_PHY_1P8_VOL_MAX);
		if (rc) {
			dev_err(motg->phy.dev, "unable to set voltage level for hsusb_gpiopull\n");
			goto put_gpiopull;
		}
		return 0;
	}
...
static int msm_otg_notify_chg_type(struct msm_otg *motg)
{
	...
	if (motg->chg_type == USB_SDP_CHARGER)
		...
		charger_type = POWER_SUPPLY_TYPE_USB_CDP;
			...
			motg->chg_type == USB_FLOATED_CHARGER || motg->chg_type == USB_T_HUB_CHARGER)
	...
	case OTG_STATE_A_HOST:
		if (test_bit(ID, &motg->inputs)) {
			...
			if((USB_CHG_STATE_DETECTED == motg->chg_state)&&(USB_T_HUB_CHARGER== motg->chg_type))
			{
				...
				{
				/*here we need to check if VBUS is on, and
				open it for pure host mode support*/
...
out:
	if (motg->is_ext_chg_dcp) {
		if (test_bit(B_SESS_VLD, &motg->inputs)) {
			msm_otg_notify_charger(motg, DCP_CHG_MAX);
		} else {
			motg->is_ext_chg_dcp = false;
			motg->chg_state = USB_CHG_STATE_UNDEFINED;
			motg->chg_type = USB_INVALID_CHARGER;
...
static void msm_id_status_w(struct work_struct *w)
{
	...
	if (motg->pdata->pmic_id_irq)
	...
	else if (motg->ext_id_irq)
		{
			...
			{
				pr_err("dhx----USB_ID_T_HUB_WITHOUT_POWER_HIGH\n");
			...
			else if(adc_id_state == USB_ID_T_HUB_WITH_POWER_HIGH)
			{
				motg->chg_type = USB_T_HUB_CHARGER;
				motg->chg_state = USB_CHG_STATE_DETECTED;
				pr_err("dhx---USB_ID_T_HUB_WITH_POWER_HIGH\n");
...
otg_get_prop_usbid_voltage_now(struct msm_otg *motg)
{
	...
	rc = qpnp_vadc_read(motg->id_vadc_dev, LR_MUX7_HW_ID, &results);//0x11=17=P_MUX2_1_1
	if (rc) {
		pr_err("Unable to read usbid rc=%d\n", rc);
		return 0;
	} else {
		pr_err("read usbid results.physical=%d\n", (int)results.physical);
		return results.physical;
	}
}

struct msm_otg *this_motg = NULL;
int hq_otg_read_state(void)
{
	return msm_otg_read_adc_id_state(this_motg);
}
				  ...
				  const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_REAL_TYPE:
		...
		else if (motg->id_state == USB_ID_GROUND){
			queue_delayed_work(motg->otg_wq, &motg->id_status_work, 0);
		}
