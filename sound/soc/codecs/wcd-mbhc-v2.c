				...
				struct snd_soc_jack *jack, int status, int mask)
{
	...
	if(!status && (jack->jack->type&WCD_MBHC_JACK_MASK)){
		...
		switch_set_state(&accdet_data,1);
	}
...
static bool get_spk_pa_status(struct snd_soc_codec *codec)
{
    struct snd_soc_card *card = codec->component.card;
	struct msm8916_asoc_mach_data *pdata = snd_soc_card_get_drvdata(card);
    return pdata->spk_ext_pa_status;
}


static int mbhc_enable_spk_ext_pa(struct snd_soc_codec *codec, int enable)
{
	struct snd_soc_card *card = codec->component.card;
	struct msm8916_asoc_mach_data *pdata = snd_soc_card_get_drvdata(card);
	...
	printk("%s: %s mbhc contrl external speaker PA\n", __func__,
...
static void wcd_correct_swch_plug(struct work_struct *work)
{
    ...
    if(get_spk_pa_status(codec)){
        printk("%s: disable ext spk pa\n", __func__);
        mbhc_enable_spk_ext_pa(codec,false);
        WCD_MBHC_REG_UPDATE_BITS(WCD_MBHC_FSM_EN, 0);
        msleep(200);
    }
    mbhc->dapm_pa_status = wcd_mbhc_is_hph_pa_on(mbhc);
    wcd_mbhc_set_and_turnoff_hph_padac(mbhc);
...
exit:
    ...
    if(mbhc->dapm_pa_status){
        wcd_mbhc_clr_and_turnon_hph_padac(mbhc);
    }
    if(get_spk_pa_status(codec)){
        mbhc_enable_spk_ext_pa(codec,true);
    }
