...
{
	const char *spk_ext_pa_left = "qcom,msm-spk-ext-pa-left";
    const char *spk_ext_pa_right = "qcom,msm-spk-ext-pa-right";
    int ret;

	printk("%s:Enter [%d,%d]\n", __func__,pdata->spk_ext_pa_left_gpio,
        pdata->spk_ext_pa_right_gpio);

    pdata->spk_ext_pa_mode = EXT_PA_MODE;

	pdata->spk_ext_pa_left_gpio = of_get_named_gpio(pdev->dev.of_node,
				spk_ext_pa_left, 0);

	if (pdata->spk_ext_pa_left_gpio < 0) {
		dev_dbg(&pdev->dev,
			"%s: missing %s in dt node\n", __func__, spk_ext_pa_left);
	} else {
        ret = gpio_request(pdata->spk_ext_pa_left_gpio, "ext_pa_left_enable");
        if (ret) {
            pr_err("%s:pdata->spk_ext_pa_left_gpio : failed to request rest gpio %d error:%d\n",
            __func__,pdata->spk_ext_pa_left_gpio, ret);
            gpio_free(pdata->spk_ext_pa_left_gpio);
            pdata->spk_ext_pa_left_gpio = -1;
        return ret;
        }
	}

    pdata->spk_ext_pa_right_gpio = of_get_named_gpio(pdev->dev.of_node,
				spk_ext_pa_right, 0);
    if (pdata->spk_ext_pa_right_gpio < 0) {
		dev_dbg(&pdev->dev,
			"%s: missing %s in dt node\n", __func__, spk_ext_pa_right);
	} else {
        ret = gpio_request(pdata->spk_ext_pa_right_gpio, "ext_pa_right_enable");
        if (ret) {
            pr_err("%s:pdata->spk_ext_pa_right_gpio : failed to request rest gpio %d error:%d\n",
            __func__,pdata->spk_ext_pa_right_gpio, ret);
            gpio_free(pdata->spk_ext_pa_right_gpio);
            pdata->spk_ext_pa_right_gpio = -1;
        return ret;
        }
    }
      gpio_direction_output(pdata->spk_ext_pa_left_gpio, 0);
      gpio_direction_output(pdata->spk_ext_pa_right_gpio, 0);

    pdata->spk_ext_pa_status = false;
    printk("%s:exit [%d,%d]\n", __func__,pdata->spk_ext_pa_left_gpio,
                                         pdata->spk_ext_pa_right_gpio);
	return 0;
}

int ext_spk_gpio_free(struct platform_device *pdev,
			struct msm8916_asoc_mach_data *pdata)
{
	printk("%s:Enter [%d,%d]\n", __func__,pdata->spk_ext_pa_left_gpio,
                                     pdata->spk_ext_pa_right_gpio);
    if(pdata->spk_ext_pa_left_gpio > 0){
            printk("%s:pdata->spk_ext_pa_left_gpio[%d] is already request,free it\n",
                __func__,pdata->spk_ext_pa_left_gpio);
            gpio_free(pdata->spk_ext_pa_left_gpio);
            pdata->spk_ext_pa_left_gpio = -1;
    }
    ...
    if(pdata->spk_ext_pa_right_gpio > 0){
            printk("%s:pdata->spk_ext_pa_right_gpio[%d] is already request,free it\n",
                __func__,pdata->spk_ext_pa_right_gpio);
            gpio_free(pdata->spk_ext_pa_right_gpio);
            pdata->spk_ext_pa_right_gpio = -1;
    }    
...
static int enable_spk_ext_pa(struct snd_soc_codec *codec, int enable)
{
    ...
    int pa_mode = pdata->spk_ext_pa_mode;
    
	if (!gpio_is_valid(pdata->spk_ext_pa_left_gpio)) {
		pr_err("%s: Invalid gpio: %d\n", __func__,
			pdata->spk_ext_pa_left_gpio);
		return false;
	}
    if (!gpio_is_valid(pdata->spk_ext_pa_right_gpio)) {
		pr_err("%s: Invalid gpio: %d\n", __func__,
			pdata->spk_ext_pa_right_gpio);
		return false;
	}
	...
	if (enable) {
        while(pa_mode > 0){  
            ...
            gpio_direction_output(pdata->spk_ext_pa_right_gpio, 0);
            udelay(2);
            gpio_direction_output(pdata->spk_ext_pa_left_gpio, 1);
            gpio_direction_output(pdata->spk_ext_pa_right_gpio, 1);
            udelay(2);
            pa_mode--;
        }
        pdata->spk_ext_pa_status = true;
	} else {
      gpio_direction_output(pdata->spk_ext_pa_left_gpio, 0);
      gpio_direction_output(pdata->spk_ext_pa_right_gpio, 0);
      pdata->spk_ext_pa_status = false;
...
parse_mclk_freq:
    ...
    printk("%s:snd_soc_register_card success!!!",__func__);
	return 0;
err:
	if (pdata->vaddr_gpio_mux_spkr_ctl)
